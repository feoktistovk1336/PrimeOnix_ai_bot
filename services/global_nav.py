...
PRIME_BUTTONS = {
    ...
    "📣 Контент", "📣 Контент Центр", "📢 Telegram", "🎯 Воронки", "🎯 Воронки IG→TG", "📬 Рассылка", "📬 Рассылки", "👥 Пользователи", "💎 Лимиты", "💎 Подписки и лимиты",
    "🔥 Viral Lab", "🔥 Вирусные идеи", "🎯 Хуки", "📈 Анализ конкурента", "📊 Аудит аккаунта", "🧠 Оценить идею", "📅 Viral план", "🤖 AI Agent",
    "📈 Статистика", "📦 Очередь", "🤖 AI Лаборатория", "🧪 Система", "🧪 Проверки", "🧪 Проверка системы", "🧭 Карта", "🧭 Карта системы",
...
class GlobalNavigationMiddleware(BaseMiddleware):
    ...
    async def _handle_button(self, message: Message, state: FSMContext, text: str) -> bool:
        ...
        if text in {"🎬 Viral Reels", "🎠 Viral Carousels", "🎯 Hook Generator", "🎁 Lead Magnet Lab", "🔥 Вирусные идеи", "🎯 Хуки", "📈 Анализ конкурента", "📊 Аудит аккаунта", "🧠 Оценить идею", "📅 Viral план", "🤖 AI Agent"}:
            if not owner:
                await message.answer("❌ Нет доступа")
                return True

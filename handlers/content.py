...
async def cancel_user_state_if_button(message: Message, state: FSMContext):
    ...
    if message.text in {"🎭 Стиль", "🎭 Стиль текста"}:
        await message.answer(
            ...
            "🧠 Обучить стилю — отправь 3–4 своих поста за раз.\n"
...
async def style_menu_handler(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "🧠 Обучить стилю — анализ 3–4 твоих поста за раз.\n"
        "🎭 Мой стиль — посмотреть сохранённый стиль.\n"
        "♻️ Сбросить стиль — начать заново.\n\n"
        "Выбери действие:",
        reply_markup=style_menu
...
async def learn_style_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        "🧠 Обучение стилю\n\n"
        "Отправь 1–4 примера своего текста или постов.\n\n"
        "Можно вставить:\n"
        "— пост из Telegram\n"
        "— продающий текст\n"
        "— описание услуги\n"
...
async def save_style_sample_handler(message: Message, state: FSMContext):
    ...
    if len(sample) < 40:
        await message.answer(
            "❌ Слишком мало текста.\n\n"
            "Отправь хотя бы один пост, ссылку с пояснением или несколько абзацев. Лучше — 3–4 поста одним сообщением. Можно обучать каждый день, профиль будет накапливаться.",
            reply_markup=style_menu,
        )
        return
...
""".strip()
    ...
    await message.answer(
        ...
        "Можно обучать ещё: хоть каждый день отправляй по 3–4 поста. "
